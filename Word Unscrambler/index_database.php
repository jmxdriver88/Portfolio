...
if( isset( $_POST[ "submit" ] ) )
{
    ...
    if( $mode == "unscramble" )
    {
        ...
        $message .= "</ul>\n<span id=\"time\">Database queries took: <b>$mSec</b> milliseconds</span>\n</div>\n";
    }
...
function unscramble( $word )
{
    ...
    {
        $database = new mysqli("localhost", "root", "", "unscrambler");
		if( mysqli_connect_error() )
		{
			exit( "Problem connecting to Database.".mysqli_connect_error() );
		}

		$query = $database->prepare( "SELECT word FROM ".$firstLetter.$length.
			" WHERE sorted = ?" );
		$query->bind_param( "s", $sortedWord );

		$query->execute();
		$query->bind_result( $matches );
